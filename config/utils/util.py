...
def torch2onnx(model, shape):
    ...
    _ = torch.onnx.export(model=model,
                          args=inputs,
    ...
    onnx.checker.check_model(onnx.load(f='weights/model.onnx'))


def onnx2caffe():
    onnx_model = onnx.load(f='weights/model.onnx')
...
def print_benchmark(shape):
    ...
    with open('weights/model.predict.pb', 'rb') as f:
        ...
        model.param_init_net.GaussianFill([],
                                          model.net.is_external_inputs[0].GetUnscopedName(),
                                          shape=shape,
                                          mean=0.0,
                                          std=1.0
                                          )
...
class RandomAugment:
    ...
    def __call__(self, image):
        if random.random() > 0.5:
            return image
        for transform in np.random.choice(TRANSFORMS, 2):
